...
const New = () => {
    ...
    const [details, setDetails] = useState({
      ...
      const createNew = async () => {
        ...
        formData.append('startTime', corper.name);
        formData.append('endTime', corper.email);
        formData.append('venue', corper.phone);
        formData.append('topic', corper.state);
        formData.append('minutes', corper.statecode);
        formData.append('type', corper.batch);
        try {
          await Axios.post("http://localhost:3030/createNew", 
          formData, {
            headers: {"Content-Type": "multipart/form-data"}
          }).then(() => {
            console.log("MinuteCreated.")
          })
        } catch (e) {
          console.log(e);
        }
