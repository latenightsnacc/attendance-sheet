...
app.post('/createNew', async (req, res) => {
    try {
            ...
            db.query('INSERT INTO notes (date, venue, topic, start_time, end_time, batch, lga, cds_group, ppa, profile_pic) VALUES(?,?,?,?,?,?,?,?,?,?)', [name,email,phone,state,statecode,batch,lga,cds,ppa,profilePic], (err, result) => {
                if(err){
                    console.log(err)
                } else {
                    res.send("Values Inserted");
